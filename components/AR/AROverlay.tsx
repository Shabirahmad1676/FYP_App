...
const AROverlay: React.FC<AROverlayProps> = ({ 
  ...
  const handleGaze = () => {
    ...
    const ViroNodeAny = ViroNode as any;

    return (
    // ReactVision 2.54.0: semanticMask for real-world occlusion
    <ViroNodeAny
      position={[0, 0, 0]}
      rotation={[-90, 0, 0]}
      scale={[0.1, 0.1, 0.1]}
      opacity={0}
      semanticMask={["person", "vehicle"]}
    ...
    >
      ...
      {/* 3D CONTENT: Animated GLB Model (ReactVision 2.54.0) */}
      {glbUrl && (
